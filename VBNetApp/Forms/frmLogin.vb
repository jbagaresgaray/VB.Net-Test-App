...
Public Class frmLogin
    ...
    Private Sub OK_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles OK.Click
        ...
        If LoginUser(txtUserName.Text, txtPassword.Text) Then
            ...
            Close()
        Else
            MessageBox.Show("Username or Password is incorrect", "Login", MessageBoxButtons.OK, MessageBoxIcon.Warning)
        End If
    End Sub
    ...
    Private Function LoginUser(ByVal UName As String, ByVal UPass As String) As Boolean
        ...
        con.Open()
        Dim cmd As New MySqlCommand("SELECT * FROM users WHERE username='" & UName & "' AND password=md5('" & UPass & "') LIMIT 1", con)
        Dim dr As MySqlDataReader = cmd.ExecuteReader()
        dr.Read()
        If dr.HasRows Then
            CURRENT_USER.fname = dr("fname").ToString()
            CURRENT_USER.userId = dr("userId").ToString()
            CURRENT_USER.lname = dr("lname").ToString()
            CURRENT_USER.mname = dr("mname").ToString()
            CURRENT_USER.username = dr("username").ToString()
            LoginUser = True
            isLogin = LoginUser
        Else
            LoginUser = False
